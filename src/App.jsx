...
const formIdx = (st) => [0, 1, 2, 3, 3][Math.max(0, Math.min(4, st))]; // egg + 3 forms; higher tiers reuse the final
...
function Creature({ species = "florn", stage = 1, vitality = 70, size = 220, skin = null, accessories = [], pose = "idle", shiny = false }) {
  ...
  if (SPRITES[species]) return <SpriteCreature species={species} stage={stage} shiny={shiny} size={size} mood={mood} />;
...
export default function App() {
  ...
  const completeDay = () => {
    ...
    let lvl = false, sank = false; const oldStage = p.stage;
    ...
    const evolved = lvl && formIdx(p.stage) !== formIdx(oldStage);
  ...
  return (
    <Shell tab={tab} setTab={setTab} flash={flash} onLogout={logout} reqCount={incoming.length} saveState={saveState}>
      ...
      {evolve && <EvolveOverlay species={evolve.species} stage={evolve.stage} name={evolve.name} shiny={evolve.shiny} onClose={() => setEvolve(null)} />}
    </Shell>
  );
}
